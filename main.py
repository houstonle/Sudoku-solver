...
class AC3:
    ...
    def consistency(self, grid, Q):
        ...
        # Implemented based on pseudocode given in assignment 4. 
        # Instead of removeal and adding to queue being done at the end each one is done when a removal (row / column / grid) is made.

        if type(Q) is not list: # For some reason Q is a list except the last element which is a tuple. I don't know why... but I'll just cast it to a list.
            q = [Q]
        else:
            q = Q
        ...
        while len(q) > 0:
            ...
            variables_assigned_row, is_inconsistent_r = self.remove_domain_row(grid, var[0], var[1])
            variables_assigned_col, is_inconsistent_c = self.remove_domain_column(grid, var[0], var[1])
            variables_assigned_unit, is_inconsistent_u = self.remove_domain_unit(grid, var[0], var[1])

            if is_inconsistent_r or is_inconsistent_c or is_inconsistent_u:
                return True

            q.extend(variables_assigned_row)
            q.extend(variables_assigned_col)
            q.extend(variables_assigned_unit)
...
class Backtracking:
    ...
    def search(self, grid, var_selector):
        ...
        Implemented according to the pseudocode shown in assignment 4.
        """
        ...
        for d in grid.get_cells()[var[0]][var[1]]:
            if grid.is_value_consistent(d, var[0], var[1]):
                ...
                # Modified to use consistency
                if ac3.consistency(grid_copy, var):
                    continue
    ...
    def pre_process(self, grid):
        """
        This method is used to pre-process the grid before running the search. 
        """
        if ac3.pre_process_consistency(grid):
            return False
...
for p in problems:
    ...
    if not BT.pre_process(g):
        print(BT.search(g, FA).print())
    time_end = time.time()
    running_time_first_available.append(time_end - time_start)
    ...
    time_start = time.time()
    if not BT.pre_process(g):
        print(BT.search(g, Mrv).print())
    time_end = time.time()
    running_time_mrv.append(time_end - time_start)

# Plot results
plotter = PlotResults()
plotter.plot_results(running_time_mrv, running_time_first_available, "Running Time Backtracking (MRV)", "Running Time Backtracking (First Available)", "running_time")